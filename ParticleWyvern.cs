	...
	public class ParticleWyvernScript : EnemyScript
	{
		...
		public IEnumerator Bolt()
		{
			for (; ; )
			{
				...
				yield return new WaitForSeconds(UnityEngine.Random.Range(1f, 4f)); //removing this timing makes the game freeze :(
			}
		}
		...
		public void Update()
		{
			if (Network.isServer)
			{
				...
				if (deathcount == 13) {
				deathcount = 0;
					EntityRegistry.Singleton["Tiers+:PlasmaDragon"].Spawn(new Vector3(MenuScript.player.transform.position.x, MenuScript.player.transform.position.y + 75f, 0f));
		...
		[RPC]
		public new void Die(int a)
		{
			if (a == 1)
			{
				deathcount++;
				// Increment your counter or whatevs
			}
			base.Die(a);
		}
